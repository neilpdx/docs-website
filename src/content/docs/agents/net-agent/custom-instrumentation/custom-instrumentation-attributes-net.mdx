...
<Callout variant="tip">
 If some of your methods still do not show up in traces after adding the `[Trace]` attribute, you may need to also add disable method inlining with [MethodImpl(MethodImplOptions.NoInlining)]
