...
<Callout variant="tip">
 If some of your methods still do not show up in traces after adding the `[Trace]` attribute, you may need to also disable method inlining for some methods with `[MethodImpl(MethodImplOptions.NoInlining)]`
