...
1. Create a new `.xml` file in the `Extensions` directory within your .NET Agent directory. The location of this directory depends on your OS:
   ...
   <CollapserGroup>
     ...
     >
       ...
       ```
       D:\home\NewRelicAgent\Framework\Extensions
       ```

       .NET Core agent:

       ```
       D:\home\NewRelicAgent\Core\Extensions
       ```
       ...
       <Collapser
       id="nuget"
       title="For NuGet agent installations"
     >
       Use the `newrelic/Extensions` folder added to your project by NuGet.
