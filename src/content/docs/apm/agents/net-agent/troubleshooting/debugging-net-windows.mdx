...
* **IIS applications**: There are **[permissions issues](/docs/agents/net-agent/troubleshooting/no-data-registry-key-permission-issues)** on your server which are preventing the application you're trying to monitor from accessing the registry keys where the variables are set. Often, you'll see application errors in the **Windows Event Viewer** related to `.NET Runtime` that could reveal further details.
...
## Step 5: Validate your newrelic.config file [#step-five]

If there's an error in the newrelic.config file (either the default config file or any you may have added to app directories) such as mal-formed XML, you may see Profiler logs but the agent could fail to start up and write agent logs. [Here](https://knowledge.newrelic.com/s/article/How-to-Validate-your-NET-APM-agent-newrelic-config-file) are some ways you can validate your config file.
